...
export default async function handler(req, res) {
  ...
  const { gradeId, seasonId } = req.query;
  if (!required(res, { gradeId, seasonId }, ["gradeId", "seasonId"])) return;

  const url = `${PLAYHQ_BASE}/v1/grades/${gradeId}/games?seasonId=${seasonId}`;
